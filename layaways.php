							...
							<div class="row">
								...
								<div class="col-md-2">
									<div class="form-group">
										...
										<select type="select" name="action" class="form-control form-control-sm" placeholder="Choose an action">

											<option value="" <?php if (empty($_GET['action'])) { echo "selected";} ?>>All</option>
											<option value="deposit" <?php if (isset($_GET['action'])){if($_GET['action'] == 'deposit' ){ echo "selected"; }}  ?>>deposit</option>
											<option value="refund" <?php if (isset($_GET['action'])){if($_GET['action'] == 'refund'){ echo "selected"; }}  ?>>refund</option>
											<option value="drawer" <?php if (isset($_GET['action'])){if($_GET['action'] == 'drawer') { echo "selected"; }}  ?>>drawer</option>
								...
								<div class="col-md-2">
									<div class="form-group">
										...
										<select type="select" name="type" class="form-control form-control-sm" placeholder="Choose an Payment Method">
										
										<option value="" <?php if (empty($_GET['type'])) { echo "selected"; } ?>>All</option>
											<option value="cash" <?php if (isset($_GET['type'])){ if($_GET['type'] == "cash"){ echo "selected"; }} ?>>Cash</option>
											<option value="credit" <?php if (isset($_GET['type'])){ if($_GET['type'] == "credit"){ echo "selected"; }} ?>>Credit</option>
										</select>
									</div>
								</div>
								<div class="col-md-2">
								  ...
								  $query = "SELECT * FROM deposits, paymentdetails, customerdetails 
									...
									if (mysqli_num_rows($result) > 0) {
									  while ($row = mysqli_fetch_assoc($result)) {
										...
										?>
											<tr>
												<td><?php echo $row['depositID']; ?></td>
												<td><?php echo $row['name']; ?></td>
												<td><?php echo $row['Deposit']; ?></td>
												<td><?php echo $row['paymentType']; ?></td>
												<td><?php echo $row['balance']; ?></td>
												<td><?php echo $row['stat']; ?></td>
												<td><?php echo date('m/d/Y g:i a', strtotime($day)); ?></td>
											</tr>
						...
						<div class="card-body">
							...
							<div class="form-group row ">
								<div class="col-xs-4 pr-2">
									<div>
										<input type="text" step="any" class="form-control form-control-sm" name="ttd" value="<?php if (!empty($totalDepo)) {
											echo $totalDepo;} ?>">
					...
					<div class="modal-dialog">
						<div class="modal-content">
							...
							<div class="modal-body">
								...
								<div class="row h4 px-5">
									<table>
										...
										<tr>
											<td>-</td>
											<td><?php echo $_GET['sold']; ?></td>
											<td></td>
										</tr>

										<tr>
											<td> </td>
											<td class="border-top border-dark"><?php echo round($amt, 2); ?></td>
								...
								<div class="text-center row px-5">
									<h4><em><?php echo 'Please deposit $' . round($amt, 2) . ' in ' . $_GET['type'] . ' payment into the bookstore'; ?></em></h4>
								</div>
							...
							<div class="modal-footer">
								...
								<a href='<?php echo "layaway_process.php?depo&ttd=$dep&count=$count&sold=$sold&type=$type"; ?>'>
